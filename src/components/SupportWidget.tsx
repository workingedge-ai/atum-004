...
const SupportWidget = () => {
  ...
  return <>
      {/* Support Button */}
      {!isOpen && <Button onClick={() => setIsOpen(true)} className="fixed bottom-6 right-6 h-14 w-14 rounded-full shadow-glow hover:scale-110 transition-transform z-50 animate-scale-in" size="icon">
          <MessageCircle className="h-6 w-6" />
        </Button>}

      {/* Support Widget */}
      {isOpen && <Card className={`fixed bottom-4 md:bottom-6 right-4 md:right-6 w-[calc(100vw-2rem)] md:w-[400px] max-w-[400px] border-border shadow-glow z-50 transition-all duration-300 ${isMinimized ? "h-14" : "h-[500px] md:h-[600px]"} animate-scale-in`}>
          {/* Header */}
          <div className="flex items-center justify-between p-3 md:p-4 border-b border-border bg-primary/10">
            ...
            <div className="flex items-center gap-1 md:gap-2">
              <Button variant="ghost" size="icon" onClick={() => setIsMinimized(!isMinimized)} className="h-7 w-7 md:h-8 md:w-8">
                <Minimize2 className="h-3 w-3 md:h-4 md:w-4" />
              </Button>
              <Button variant="ghost" size="icon" onClick={() => setIsOpen(false)} className="h-7 w-7 md:h-8 md:w-8">
          ...
          {/* Content */}
          {!isMinimized && <Tabs defaultValue="chat" className="flex-1 flex flex-col">
              ...
              <TabsContent value="chat" className="flex-1 flex flex-col m-0 h-[410px] md:h-[490px]">
                ...
                <div className="flex-1 overflow-y-auto p-3 md:p-4 space-y-3 md:space-y-4">
                  {messages.map(message => <div key={message.id} className={`flex ${message.sender === "user" ? "justify-end" : "justify-start"} animate-fade-in`}>
                      <div className={`max-w-[85%] md:max-w-[80%] rounded-lg p-2 md:p-3 ${message.sender === "user" ? "bg-primary text-primary-foreground" : "bg-muted"}`}>
                ...
                {/* Input */}
                <div className="p-3 md:p-4 border-t border-border py-[16px]">
                  <div className="flex gap-2">
                    
                    <Input value={inputMessage} onChange={e => setInputMessage(e.target.value)} onKeyPress={e => e.key === "Enter" && handleSendMessage()} placeholder="Type your message..." className="flex-1 text-xs md:text-sm h-8 md:h-10" />
                ...
                <div className="flex-1 overflow-y-auto p-3 md:p-4 space-y-3 md:space-y-4">
                  ...
                  <Card className="p-4 bg-muted/30 border-border">
                    ...
                    <div className="space-y-3">
                      <Input value={ticketSubject} onChange={e => setTicketSubject(e.target.value)} placeholder="Subject" className="text-sm" />
                      <Textarea value={ticketDescription} onChange={e => setTicketDescription(e.target.value)} placeholder="Describe your issue..." className="text-sm min-h-[80px]" />
                  ...
                  <div className="space-y-2">
                    ...
                    {tickets.map(ticket => <Card key={ticket.id} className="p-3 border-border hover:shadow-glow transition-all cursor-pointer animate-fade-in">
                        <div className="flex justify-between items-start mb-2">
                          <h5 className="font-semibold text-sm">{ticket.subject}</h5>
                          <Badge variant="secondary" className={ticket.status === "Resolved" ? "bg-green-500/20 text-green-700" : ticket.status === "In Progress" ? "bg-blue-500/20 text-blue-700" : "bg-orange-500/20 text-orange-700"}>
                            {ticket.status}
                          </Badge>
                        </div>
